...
import hw2.GamePhraseList;
import hw2.Player;
import hw2.GameText;
import hw2.GameWheel;
import hw2.Game;
...
{ 
  ...
  public static void main(String[] args)
  {
    ...
    WheelOfFortuneUI.start(name0, name1, filename, seed); 
  }
  ...
  /**
   * Starting character of the alphabet, duh.
   */
   ...
   * The word generator.
   */
  private final GamePhraseList phraseSource;
   ...
   * @param filename
   *   filename for the GamePhraselist to be used
   * @seed
   *   seed for the game's random number generator
   */
  ...
  {    
    ...
    {   
      ...
      GamePhraseList phraseSource = new GamePhraseList(filename);
      
      // create an instance of our JPanel subclass and 
      // add it to the frame
      frame.getContentPane().add(new WheelOfFortuneUI(player0Name, player1Name, phraseSource, seed));
   ...
   */
  private WheelOfFortuneUI(String player0Name, String player1Name, GamePhraseList phraseSource, int seed) throws FileNotFoundException
  {
    ...
    game.startRound(0, new GameText(phrase));
  ...
  private class PlayAgainHandler implements ActionListener
  {
    ...
    {
      String phrase = phraseSource.getPhrase(rand.nextInt(phraseSource.getSize()));
      game.startRound(1 - game.whoseTurn(), new GameText(phrase));
      resetAll();
      for (int i = 0; i < 26; ++i)
      {
        guesses[i] = false;   
